...
def collate_fn(data):
    ...
    for key in data[0]["x"]["image_input"]:
        new_image_input[key] = torch.cat(
            [d["x"]["image_input"][key] for d in data], dim=0
        )
    bounding_boxes = [
        torch.permute(d["x"]["bounding_boxes"], (1, 2, 0)) for d in data
    ]
    bounding_boxes = pad_sequence(bounding_boxes, batch_first=True).squeeze(
        dim=-1
    )
...
class MedGeeseDataset(Dataset):
    ...
    def __init__(self, items: list[str], model_path: str, is_testing: bool, from_mem: bool):
        ...
        if from_mem:
            ...
            #self.file_dir = {idx: path for idx, path in enumerate(items)}
            self.examples = {}
            ...
            #self.imgs = {idx: torch.load(tensor_dir + f"/images/{stem}", weights_only=False) for idx, path in enumerate(stem)}
            print("done loading...")
            del split_basenames
            del stem
        self.items = items
    ...
    def __getitem__(self, idx: int):
        ...
        """
        if self.from_mem:
            #filename = self.file_dir[idx]
            #path = self.items[idx]
            path, (mask, label, candidate_text, bboxes) = self.examples[idx]
            #print(path)
            ...
            #(mask, label, candidate_text, bboxes) = torch.load(
